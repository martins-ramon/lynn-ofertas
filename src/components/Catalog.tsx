...
import { agents, classificationLabel, getPortfolioItem, normalize, type Agent } from '../data/catalog';
...
export default function Catalog({ onAgent }: { onAgent: (agent: Agent) => void }) {
  ...
  return <section className="catalog-section section-shell" id="ofertas" aria-labelledby="catalog-title">
    <SectionHeading index="04" eyebrow="AGENTES TOTVS · EXEMPLOS DE APLICAÇÃO" title={<span id="catalog-title">Encontre uma possibilidade.<br /><span className="muted-heading">Imagine o impacto.</span></span>} text="A oferta Agentes TOTVS reúne a produção dos times de Produtos, OES e franquias. Explore os exemplos de automação dos materiais de referência."><div className="catalog-stat"><strong>55<span>+</span></strong><span>exemplos nos materiais<br />e um portfólio em expansão</span></div></SectionHeading>
    ...
    <div className="agent-grid">{results.slice(0, limit).map(agent => {
      ...
      return <button className={`agent-card accent-${item.accent}`} key={agent.id} onClick={() => onAgent(agent)}><div className="agent-card-top"><span className="micro">{agent.segment}</span><ArrowUpRight size={17} /></div><h3>{agent.name}</h3><span className="classification-label">{classificationLabel(item)}</span><p>{agent.description}</p><div className="agent-products">{agent.products.length > 0 ? agent.products.map(product => <span key={product}>{product}</span>) : <span>Segmento {agent.segment}</span>}</div><div className="agent-card-footer"><span><PortfolioIcon id={agent.family} size={13} />{agent.family === 'standard' ? 'Padrão · Produtos' : 'OES'}</span><ModelBadge item={item} /></div><div className="agent-status"><span className={`tiny-dot ${agent.status === 'Disponível no material' ? 'available' : ''}`} />{agent.status}</div></button>;
    })}</div>
