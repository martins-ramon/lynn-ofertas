...
export default function WalletJourney() {
  ...
  return <section className="wallet-section" id="wallet" aria-labelledby="wallet-title"><div className="section-shell">
    ...
    <div className="taas-explainer"><div><span className="micro">TaaS · TASK AS A SERVICE</span><h3>O produto executa. A oferta é consumida.</h3><p>A volumetria e a métrica do produto de IA orientam o consumo da oferta T-Coins. Start e faixas organizam os créditos. OES tem ofertas próprias; Garden tem modelo a definir.</p></div><ol><li><CheckCheck size={21} /><span>Produto executa a tarefa</span></li><li><Coins size={21} /><span>Uso é medido</span></li><li><WalletIcon size={21} /><span>T-Coins são debitados</span></li></ol></div>
    ...
    {assumptions && <Modal titleId="assumptions-title" onClose={() => setAssumptions(false)}><span className="eyebrow">COMO LER A DEMONSTRAÇÃO</span><h2 id="assumptions-title">Um exemplo para entender.<br />Não uma tabela comercial.</h2><p className="modal-lead">O ciclo ilustra a reunião de 09/09/2026, com dois produtos da família Agentes Padrão. Os números são fictícios.</p><ul className="detail-list"><li>Start com 100 T-Coins; capacidades totais de 200, 350 e 500 T-Coins nas faixas fictícias, incluindo a franquia Start.</li><li>Documentos do Fluig a 10 T-Coins por documento e Liberação de crédito do Protheus a 15 por pedido. Métricas e tarifas reais ainda a definir.</li><li>Na demonstração, a troca complementa a diferença de créditos entre faixas, e o lote válido que vence primeiro é consumido primeiro. São hipóteses ilustrativas.</li><li>Os dias de corte e renovação são fictícios. A validade de 180 dias por entrada vem da reunião.</li><li>Regras detalhadas de redução, cancelamento, preços, alertas e alocação de saldo na troca dependem de definição.</li><li>OES tem contratação própria hoje. Garden tem modelo comercial em discussão e não gera consumo nesta simulação.</li></ul><div className="source-note">Transcrição de 09/09/2026, 00:00:00–00:33:10; definição canônica de oferta/produto e contexto Garden. O extrato visual foi descrito como em desenvolvimento.</div></Modal>}
  </div></section>;
}
